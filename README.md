# 书声本地 · ebook2tts-local
  ...
  > **v1.0.1** ｜ 作者 **Kermond** ｜ LICENSE **MIT** ｜ 包名 `com.kermond.ebook2tts`
...
- 当前 **`1.0.1`**（正式版；`versionCode=2`，`minSdk 27` / `targetSdk 34` / `compileSdk 36`）。
...
> 本版本为 **v1.0.1**。以下内容为**如实披露**：受测试条件限制未能完成的验证项，以及已知的限制与缺陷。
...
| ~~正式包 ABI 未完全收敛~~ **已修复**（v1.0.1） | 正式包此前含 `x86_64` 运行库（约 36 MB 冗余） | 已修复：发布流程对正式包剔除 `x86_64`（5 个运行库）并**重新对齐与签名**，核验 `native-code: arm64-v8a` 仅此一项；体积 **243 MB → 206 MB**；签名与验签通过（与既有发布证书一致） |
