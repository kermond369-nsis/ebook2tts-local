...
| 项 | 状态 | 说明 |
|---|---|---|
| 内存释放链路的**真机**取证 | **已完成**（2026-09-19） | 真机（SDM845 / Android 14）实测：释放前 **PSS 414,511 KB → 释放后 128,919 KB（−279 MB，−68.9%）**；预览在播时**正确跳过**（`skipped\|preview_active`、`released=false`）；释放后**新文本合成重新加载模型**（`loaded … threads=2`，`synth 52.1s`）并回升至 **414,408 KB** ⇒ 释放可恢复。机制级另在模拟器验证（触发送达、未加载时跳过） |
| 低配设备**功耗**对照 | **未进行（设备已撤出）** | 空闲 / 本地合成 / 在线合成 / 纯播放四组对照方案已设计；用于该测量的第二台测试机（红米 Note 12 Turbo）已从调试机名单撤出，测量未进行 |
