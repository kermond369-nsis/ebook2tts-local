...
### P7-007 ｜ 依赖陈旧 —— **关闭（证据：直接依赖全部最新）**（2026-09-17 00:27）

门禁实测（`flutter pub outdated`，附 `p7-green-gate.log`）：
```
material_color_utilities  *0.13.0  *0.13.0  *0.13.0  0.13.1
meta                      *1.18.3  1.19.0   1.19.0   1.19.0
test_api                  *0.7.12  *0.7.12  *0.7.12  0.7.14
vector_math               *2.4.0   2.4.2    2.4.2    2.4.2
...
## E8 ｜ agy 第 5 轮复核（终审）处置记录（2026-09-17 00:5x）

**复核结论**：agy 出"**多处严重红线违反**"的严厉意见。逐条判定如下（**不盲从、不盲拒**）：

| agy 意见 | 我方判定 | 处置 |
|---|---|---|
| ① 常驻后端=内存泄漏（红线1） | **驳回（理由说明）** | 常驻是**甲方批准的设计**（"不做主动释放"）；红线1原文是"绝不为内部原因中止在途朗读请求"，非 agy 所述"禁止常驻"。**但**：Native 341 MB 的常驻成本应记录，并列入"内存压力下降级释放"后续项 |
| ② release 缺 `key.properties` 回退调试密钥 = 致命 | **采纳** ✅ | **已改**：release 变体缺凭据 ⇒ **构建失败**（`GradleException`）；debug 不受影响 |
| ③ 线程上限=性能核数与实测（2 最快）背离 | **部分采纳** | **不改上限公式**（模拟器 4~5 更优、IM-522 同构核 5 更优，一刀切 2 反而伤同构核）；**保留**保守 4 + 用户可设 2；"进一步自适应"列后续项（已在 ADR-018 §15.3 记录） |
| ④ 玄戒 `O1` 短串误伤 + fail-open | **采纳** ✅ | **已改**：豁免必须 **厂商命中小米系**（Xiaomi/Redmi/POCO/XRING）**且**型号串词边界命中 `O1/O3`；新增回归用例（`SM6115-O1`/`POCO1` 均**不得**被豁免） |
| ⑤ P7-005 未核实 detekt 规则即关闭 | **采纳** ✅ | **已核实**：detekt 1.23.8 默认配置 `UnusedPrivateMember/UnusedPrivateClass/UnusedParameter/UnusedPrivateProperty` **均 `active: true`** ⇒ 扫描 0 命中**成立**；P7-005 关闭依据补强 |
| ⑥ 红线 6~9 未触及 | 同意 | 无动作 |

**测试**：core **87/87**（新增玄戒误豁免回归 1 例，共 8 例）全绿。
**证据**：`/root/work/p7-evidence/agy-p7-final-review.log`（原始复核意见，随交付归档）。
