...
## 附：凭据处理事故登记（2026-09-16，我方过失）

- **经过**：为测在线延迟，用 `strings` 读取 App 的 MMKV 配置并按前缀掩码（`sk-|Bearer |key=`）过滤输出；
  该服务 plan 类密钥前缀为 `tp-`，**未被掩码命中** ⇒ 密钥明文出现在一次后台任务的输出中。
- **影响**：该密钥视为**已泄露**，应**轮换/吊销**（用户处置）。
- **处置**：① 后续凭据提取只输出长度/前 2 字符，绝不回显原始内容；② 已把"前缀无关掩码 + 禁回显原始配置"
  写入 `security-lessons` 技能；③ 本报告与所有交付文档中不含密钥明文。
